...
test.suite(import.meta.url.replace(/^.*?\/(?<name>[^/]*)$/u, "$<name>"), () => {
  ...
  test("name and version", (t) => {
    ...
    const logMessage = (/** @type {string} */ msg) => {
      ...
      if (match) {
        ...
        t.assert.equal(name, packageJson.name);
        t.assert.equal(version, packageJson.version);
      }
    };
    const logError = (/** @type {string} */ msg) => t.assert.fail(`message logged: ${msg}`);
    return markdownlintCli2({
      "argv": [],
      logMessage,
      logError
    }).
      then((exitCode) => t.assert.equal(exitCode, 2));
  });
  ...
  test("README files", (t) => {
    t.plan(1);
    const uncalled = (/** @type {string} */ msg) => t.assert.fail(`message logged: ${msg}`);
    const argv = [
      "CHANGELOG.md",
      "README.md",
      "./doc/OutputFormatters.md",
    ...
    return markdownlintCli2({
      ...
      then((exitCode) => t.assert.equal(exitCode, 0));
  });
  ...
  test("validateExampleObjectsMatch", async (t) => {
    ...
    t.assert.deepEqual(yamlExample, jsonExample);
    ...
    t.assert.deepEqual(tomlExample, jsonExample);
  });

  test("parser precedence", async (t) => {
    t.plan(4);
    /** @type { [ string, string ][] } */
    const files = [
      [ "/json", "{ \"value\": true }" ],
  ...
  test("main options default", (t) => {
    ...
    return Promise.all([
      ...
      then((exitCodes) => {
        ...
        t.assert.equal(exitCode0, 0);
        t.assert.equal(exitCode1, 1);
      });
  });
  ...
  test("main options override", (t) => {
    ...
    return markdownlintCli2({
      ...
      then((exitCode) => t.assert.equal(exitCode, 1));
  });
  ...
  test("custom fs, extension scenario for untitled", (t) => {
    ...
    const readFile = (/** @type {string} */ file) => {
      t.assert.equal(path.basename(file), ".markdownlint-cli2.jsonc");
    ...
    const writeFile = () => {
      t.assert.fail("writeFile called");
    };
    return markdownlintCli2({
      "nonFileContents": {
        "name": "Text"
  ...
  test("--help", (t) => {
    ...
    return markdownlintCli2({
      ...
      then((exitCode) => {
        ...
        t.assert.match(stdouts[0], /^markdownlint-cli2 v/u);
      });
  });
  ...
  test("-- stops matching parameters per POSIX Utility Conventions 12.2 Guideline 10", async (t) => {
    ...
    const scenario = async (/** @type {string[]} */ argv, /** @type {number} */ exitCode) => t.assert.equal(
