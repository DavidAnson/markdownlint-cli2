...
```bash
docker run -v $PWD:/workdir davidanson/markdownlint-cli2:v0.23.3 "**/*.md" "#node_modules"
```
...
- By default, `markdownlint-cli2` will execute within the `/workdir` directory
  *inside the container*. So, as shown above, [bind mount][docker-bind-mounts]
  ...
  - A custom working directory can be specified with Docker's `-w` flag:
    ...
    ```bash
    docker run -w /myfolder -v $PWD:/myfolder davidanson/markdownlint-cli2:v0.23.3 "**/*.md" "#node_modules"
    ```
